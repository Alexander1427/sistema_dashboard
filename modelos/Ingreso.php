...
public function insertar($idproveedor,$idusuario,$tipo_comprobante,$num_comprobante,$fecha_hora,$total_compra,$idarticulo,$cantidad,$precio_compra,$precio_venta){
	$sql="INSERT INTO ingreso (idproveedor,idusuario,tipo_comprobante,num_comprobante,fecha_hora,total_compra,estado) VALUES ('$idproveedor','$idusuario','$tipo_comprobante','$num_comprobante','$fecha_hora','$total_compra','Aceptado')";
...
public function mostrar($idingreso){
	$sql="SELECT i.idingreso,i.fecha_hora as fecha,i.idproveedor,p.nombre as proveedor,u.idusuario,u.nombre as usuario, i.tipo_comprobante,i.serie_comprobante,i.num_comprobante,i.total_compra,i.impuesto,i.estado FROM ingreso i INNER JOIN persona p ON i.idproveedor=p.idpersona INNER JOIN usuario u ON i.idusuario=u.idusuario WHERE idingreso='$idingreso'";
